...
import { serverProperties } from "bdsx/serverproperties";
    ...
    if (config.modules.seedhider === true) {
        if (serverProperties["client-side-chunk-generation-enabled"] === true) {
            console.warn(`Seed hider causes issues when client-side-chunk-generation-enabled is set to true in server.properties please disable it or set seedhider to false in config.json for no further issues`)
        }
